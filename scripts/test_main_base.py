...
def test_precharge(hil):
    ...
    # Outputs
    v_bat = hil.aout("Main", "VBatt")
    v_mc  = hil.aout("Main", "Voltage MC Transducer")

    # Inputs
    # pchg_cmplt = hil.mcu_pin("Main", "PrechargeComplete") # TODO
    not_pchg_cmplt_delayed = hil.din("Main", "NotPrechargeCompleteSchmitt")
    # v_bat_mcu = hil.daq_var("Main", "Varname") # TODO
    # v_mc_mcu  = hil.daq_var("Main", "Varname") # TODO
    # pchg_mux  = hil.daq_var("Main", "Varname") # TODO

    # Initial State
    reset_pchg(v_bat, v_mc)
    time.sleep(2.5)
    # TODO hil.check(pchg_cmplt.state == 0, "Precharge not complete on startup")
    ...
    hil.check_within(t, PCHG_COMPLETE_DELAY_S, 0.25, f"Precharge complete delay of {t}s close to expected {PCHG_COMPLETE_DELAY_S}s")

    hil.end_test()
    return
    # TODO: continue once have MCU pin or measure from SDC direct
    ...
    for v in [ACCUM_MIN_V, ACCUM_NOM_V, ACCUM_MAX_V]:
        ...
        v_bat = tiff_hv_to_lv(v)
        hil.check(pchg_cmplt.state == 0, "Precharge Complete Low at Initial State")

        start = tiff_hv_to_lv(0.0)
        stop  = tiff_hv_to_lv(v)
        step  = tiff_hv_to_lv(1.0)
        thresh = utils.measure_trip_thresh(v_mc, start, stop, step, 0.010,
                                           pchg_cmplt, is_falling=False)
        thresh_hv = tiff_lv_to_hv(thresh)
        print(f"Precharge triggered at {thresh_hv / v * 100:.2}% ({thresh_hv:.2}V) of vbat={v}.")
        hil.check_within(thresh_hv / v, R_PCHG_V_BAT_THRESH, 0.01, f"Precharge threshold of {R_PCHG_V_BAT_THRESH*100}% at vbat = {v}V")
...
def test_bspd(hil):
    ...
    # Outputs
    brk_fail = hil.dout("Main", "Brake Fail")
    brk_stat = hil.dout("Main", "Brake Status")
    c_sense  = hil.aout("Main", "Current Sense C1")

    # Outputs to set SDC status to okay
    ams_stat  = hil.dout("Main", "BMS-Status-Main")
    imd_stat  = hil.dout("Main", "IMD_Status")

    # Inputs
    bspd_ctrl = hil.din("Main", "SDC3") # Assuming IMD and AMS closed
...
def test_imd(hil):
    ...
    # Inputs
    imd_ctrl  = hil.din("Main", "SDC3") # assuming AMS and BSPD closed
...
def test_ams(hil):
    ...
    # Inputs
    ams_ctrl  = hil.din("Main", "SDC3") # assumes IMD and BSPD closed
...
def test_tsal(hil):
    ...
    # Inputs
    tsal = hil.din("Main", "TSAL+")
    lval = hil.din("Main", "LVAL+")
...
if __name__ == "__main__":
    ...
    hil.load_config("config_main_base_bench.json")
    hil.load_pin_map("per_24_net_map.csv", "stm32f407_pin_map.csv")

    power = hil.dout("Arduino2", "RLY1")
